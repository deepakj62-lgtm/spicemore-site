...
export async function onRequest(context) {
  ...
  try {
    const { employee, type, from_date, to_date, days, reason, employee_email } = await request.json();
    ...
    if (data.managerEmail && env.RESEND_API_KEY) {
      ...
      await resend.emails.send({
        from: FROM_EMAIL, to: data.managerEmail,
...
<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#f5f1ed;padding:24px">
  ...
  <div style="background:#fff;padding:28px;border-radius:0 0 10px 10px;border:1px solid #ddd;border-top:none">
    ...
    return json({ ok: true, id, application }, { status: 201 });
  } catch (err) {
